...
deg = 5
l = 15
ts = (None, smooth, running_average_with_custom_length(l), polyfit(1), polyfit(deg), composite(polyfit(deg), derivative), composite(shift(2), ratios, remove_outliers), composite(shift(2), polyfit(deg), ratios, remove_outliers))
names = ("Temperature in C (shifted down by 8.61)", 'Smoothed', 'Running Average with Length ' + str(l), 'Linear Regression', 'Polyfit of Degree ' + str(deg), 'Derivative of polyfit', 'Ratios', 'Ratios on Polyfit')
